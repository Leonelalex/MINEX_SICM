...
{
    public class SICM_TIPO_CEJA_REPO : RepositoryBase<SICM_TIPO_CEJA>, ISICM_TIPO_CEJA_REPO
    {
        // CRUD --> Create, Read, Update, Delete
        private readonly ISICM_DBContext _dbContext;
        public SICM_TIPO_CEJA_REPO(ISICM_DBContext dBContext) : base(dBContext)
        {
        ...
        public async Task<bool> ExistAsync(int id)
        {
            try
            {
                if (await _dbContext.SICM_TIPO_CEJA.CountAsync(x => x.Codigo == id) > 0)
                    return true;
                else
                {
                    SICM_TIPO_CEJA newcatalogo = new SICM_TIPO_CEJA();
                    newCatalogo newinfo = new newCatalogo();

                    newinfo = await CatalogoService.mpCatalogos(ConstantsEx.SICM_TIPO_CEJA, id);
                    ...
                    if (newinfo.CODIGO != 100 && newinfo.NOMBRE != null)
                    {
                        ...
                        await _dbContext.SICM_TIPO_CEJA.AddAsync(newcatalogo);
