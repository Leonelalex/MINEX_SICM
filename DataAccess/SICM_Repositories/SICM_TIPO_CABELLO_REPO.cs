...
{
    public class SICM_TIPO_CABELLO_REPO : RepositoryBase<SICM_TIPO_CABELLO>, ISICM_TIPO_CABELLO_REPO
    {
        private readonly ISICM_DBContext _dbContext;
        public SICM_TIPO_CABELLO_REPO(ISICM_DBContext dBContext) : base(dBContext)
        {
        ...
        public async Task<bool> ExistAsync(int id)
        {
            try
            {
                if (await _dbContext.SICM_TIPO_CABELLO.CountAsync(x => x.Codigo == id) > 0)
                    return true;
                else
                {
                    SICM_TIPO_CABELLO newcatalogo = new SICM_TIPO_CABELLO();
                    newCatalogo newinfo = new newCatalogo();

                    newinfo = await CatalogoService.mpCatalogos(ConstantsEx.SICM_TIPO_CABELLO, id);
                    ...
                    if (newinfo.CODIGO != 100 && newinfo.NOMBRE != null)
                    {
                        ...
                        await _dbContext.SICM_TIPO_CABELLO.AddAsync(newcatalogo);
