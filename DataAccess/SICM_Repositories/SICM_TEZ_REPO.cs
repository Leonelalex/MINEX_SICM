...
{
    public class SICM_TEZ_REPO : RepositoryBase<SICM_TEZ>, ISICM_TEZ_REPO
    {
        private readonly ISICM_DBContext _dbContext;
        public SICM_TEZ_REPO(ISICM_DBContext dBContext) : base(dBContext)
        {
        ...
        public async Task<bool> ExistAsync(int id)
        {
            try
            {
                if (await _dbContext.SICM_TEZ.CountAsync(x => x.Codigo == id) > 0)
                    return true;
                else
                {
                    SICM_TEZ newcatalogo = new SICM_TEZ();
                    newCatalogo newinfo = new newCatalogo();

                    newinfo = await CatalogoService.mpCatalogos(ConstantsEx.SICM_TEZ, id);
                    ...
                    if (newinfo.CODIGO != 100 && newinfo.NOMBRE != null)
                    {
                        ...
                        await _dbContext.SICM_TEZ.AddAsync(newcatalogo);
